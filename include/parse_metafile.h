...
int find_keyword(char *keyword, long *position);	//在种子文件中查找某个关键字
...
int get_pieces();				//获取每个piece的哈希值

int is_multi_files();			//判断下载的文件是单个文件还是多个文件
int get_file_name();			//获取文件名，对于多文件获取的是目录名
int get_file_length();			//获取待下载文件的总长度
int get_files_length_path();	//获取文件的路径和长度
...
int parse_metafile(char *metafile);					//调用本文件定义的函数，完成解析种子文件
