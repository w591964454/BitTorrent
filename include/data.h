...
typedef struct _Btcache {
	...
	unsigned char read_write;	//事发送给peer的数据还是接收到的数据,若数据是从硬盘读出，read_write值为0；若数据将要写入硬盘，read_write值为1
	unsigned char is_full;	//缓冲区是否已满
	unsigned char is_writed;	//缓冲区的数据是否已写入到硬盘中
...
int create_files();						//根据种子文件中信息创建保存下载数据的文件

//判断一个Btcache结点中的数据要写入到哪个文件以及具体位置,并写入
int write_btcache_node_to_harddisk(Btcache *node);
//从硬盘读取一个slice的数据存放到缓冲区中，在peer需要时发送给peer
