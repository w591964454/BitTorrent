...
char	*pieces = NULL;				//保存每个piece的哈希值，每个哈希值为20字节
int		pieces_length = 0;			//缓冲区pieces的长度

int		multi_file = 0;				//指明是单文件还是多文件
char	*file_name = NULL;			//对于单文件，存放文件名；对于多问件，存放目录名
long long file_length = 0;			//存放待下载文件的总长度
Files	*files_head = NULL;			//只对多文件种子有效存放各个文件的路径和长度
...
//解析种子文件，将种子文件的内容读入到全局变量metafile_content所指向的缓冲区中以方便处理
...
//从种子文件中查找某个关键字, position用于返回关键字第一个字符所在的下标
...
int read_announce_list()
{
	...
	} else {	//如果有13:announce-list关键字就不用处理8:announce关键字
...
//连接某些Tracker时会返回一个重定向的URL，需要连接该URL才能获取peer
int add_an_announce(char *url)
{
	Announce_list *p = announce_list_head, *q;

	//若参数指定的URL在Tracker列表已存在，则无需添加
	while (p != NULL) {
...
//判断是下载单个文件还是多个文件，若有关键字“5:files”则说明下载的是多个文件
int is_multi_files()
{
	long	i;
...
//获取每个piece的hash值，并保存到pieces所指向的缓冲区中
int get_pieces()
{
	long	i;
...
//获取待下载文件的文件名，如果下载的是多个文件，则获取的是目录名
int get_file_name()
{
	long	i;
	int		count = 0;
