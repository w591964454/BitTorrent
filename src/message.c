...
//如果45秒未给某peer发送出消息，则发送keep_alive消息
#define KEEP_ALIVE_TIME		45

extern Bitmap *bitmap;			//在bitmap.c中定义， 指向己方的位图
extern char info_hash[20];		//在parse_metafield.c中定义，存放info_hash
extern char peer_id[20];		//在parse_metafield.c中定义，存放peer_id
...
int create_have_msg(int index, Peer *peer)
{
	...
	if (len < 9)
		return -1;	//have消息的长度固定为9
	memset(buffer, 0, 9);
	buffer[3] = 5;
	buffer[4] = 4;
	int_to_char(index, c);	//index为piece下标
	buffer[5] = c[0];
	buffer[6] = c[1];
	buffer[7] = c[2];
	buffer[8] = c[3];
...
int is_complete_message(unsigned char *buff, unsigned int len, int *ok_len)
{
	...
	unsigned char chock[5] = {0x0, 0x0, 0x0, 0x1, 0x0};
	unsigned char unchock[5] = {0x0, 0x0, 0x0, 0x1, 0x1};
	...
	*ok_len = 0;
	...
	for (i = 0; i < len; ) {
		if (i + 68 <= len && memcmp(&buff[i], btkeyword, 20) == 0)
			...
			i += 5;
		else if (i + 9 <= len && memcmp(&buff[i], have, 9) == 0)
			i += 9;
		else if (i + 17 <= len && memcmp(&buff[i], request, 17) == 0)
			i += 17;
		else if (i + 17 <= len && memcmp(&buff[i], cancel, 17) == 0)
			i += 17;
		else if (i + 7 <= len && memcmp(&buff[i], port, 7) == 0) 
			i += 7;
...
int process_handshake_msg(Peer *peer, unsigned char *buff, int len)
{
	...
	//记录最近收到该peer消息的时间
	//若一定时间内未收到来自该peer的任何消息，则关闭连接
	peer->start_timestamp = time(NULL);
	return 0;
}

//处理刚刚接收的来自peer的keepv_alive消息
...
int process_choke_msg(Peer *peer, unsigned char *buff, int len)
{
	...
	if (peer->state != CLOSING && peer->peer_choking == 0) {
		...
		peer->down_count = 0;			//将最近从该peer处下载的字节清零
		peer->down_rate = 0;			//将最近从该peer处下载的速度清零
	}
...
int process_unchoke_msg(Peer *peer, unsigned char *buff, int len)
{
	if (peer == NULL || buff == NULL)
		return -1;
	//若原来处于choke状态且与choke的连接未被关闭
	if (peer->state != CLOSING && peer->peer_choking == 1) {
		peer->peer_choking = 0;
		//若对该peer有兴趣，则构造request消息请求peer发送数据
...
int process_interested_msg(Peer *peer, unsigned char *buff, int len)
{
	...
	if (peer->state != CLOSING && peer->state == DATA) {
		...
		if (peer->am_choking == 0)
			create_chock_interested_msg(1, peer);
	}
...
//主动创建发送给peer的消息，而不是等收到某个消息后再创建响应消息
int create_response_message(Peer *peer)
{
	if (peer == NULL)
		return -1;
	if (peer->state == INITIAL) {//处于Intial状态时主动发送握手消息
		create_handshake_msg(info_hash, peer_id, peer);
