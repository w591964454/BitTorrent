...
extern unsigned char info_hash[20];			//存放 infov_hash
...
#ifdef DEBUG
	//printf("http encode:%s\n", out);
#endif

	return 0;
}

//获取 Tracker URL 中的主机名部分
...
int parse_tracker_response1(char *buffer, int ret, char *redirection, int len)
{
	...
	//获取返回的 peer 数，关键词"5:peers"之后为各个 Peer 的 IP 和端口
	for (i = 0; i < ret - 7; i++) {
		if (memcmp(&buffer[i], "5:peers", 7) == 0) {
			i = i + 7;
			break;
	...
	//将每个 peer 的 IP 和端口保存到 peer_addr_head 指向的链表中
	for (; count != 0; count--) {
